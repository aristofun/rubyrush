...
#
# UPD: Начиная с 30 мая 2022 года Google больше не поддерживает сторонние
# приложения и устройства, которые предлагают войти в аккаунт Google только с
# помощью имени пользователя и пароля. Поэтому вначале необходимо настроить в
# аккаунте Google вход с помощью пароля приложения.
# Инструкция: https://support.google.com/accounts/answer/185833
# После получения 16-значного пароля приложения, его следует вводить вместо
# пароля от вашей почты (без пробелов).
#
# via: :smtp,
# via_options: {
...
#
# Или просто заведите тестовый ящик на одном из сервисов, ради такого дела :)
)
